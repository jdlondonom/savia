...
export function InviteForm({ token, invitation, turnstileSiteKey }: { token: string; invitation: InvitationPreview; turnstileSiteKey: string | null }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      ...
      <label className="block"><span className="mb-1.5 block text-xs font-extrabold uppercase tracking-[.08em] text-[#687970]">Crea una contraseña</span><input required name="password" type="password" autoComplete="new-password" placeholder="Mínimo 12 caracteres" className="w-full rounded-xl border border-[#d6e1da] px-4 py-3 text-sm" /></label>
